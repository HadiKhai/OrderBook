 ...
 **/
contract OrderBook is Initializable,ReentrancyGuardUpgradeable, IOrderBook{
    ...
    function getOrdersByMakeAsset(address asset) public view returns (Order[] memory){
        Order[] memory order = new Order[](_currentOrderId.current());
    ...
    function getOrdersByTakeAsset(address asset) public view returns (Order[] memory){
        Order[] memory order = new Order[](_currentOrderId.current());
    ...
    function getRemainingAmount(uint id) public view returns (uint _remaining){
        Bid[] storage bids = _orderBids[id];
    ...
    function cancelOrder(uint256 id)  external  onlyMaker(id) nonReentrant returns(bool success){
        ...
        require(result, "Refund Market Failed");
