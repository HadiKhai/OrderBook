...
module.exports = async (deployer, network, accounts) => {
    ...
    const existingOrderBook = await OrderBook.deployed();
    const orderBook = await upgradeProxy(existingOrderBook.address, OrderBookV2, { deployer });
    await fs.appendFile(__dirname +'/../.env.'+network, "APP_ORDER_BOOK_V2_ADDRESS="+orderBook.address+os.EOL)

    if (orderBook) {
        console.log(
            `Deployed: OrderBookV2
       network: ${network}
